...
namespace JoystickSample
{
    public partial class canvas : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            int width = 476;
            int height = 316;
            Bitmap sourceBMP = (Bitmap)Bitmap.FromFile("ImgPrelucrata.bmp");
            Bitmap result = new Bitmap(width, height);
            Bitmap display = new Bitmap(width, height);
            Graphics q = Graphics.FromImage(display);
            
           
            using (Graphics g = Graphics.FromImage(result))
                g.DrawImage(sourceBMP, 0, 0, width, height);
            Bitmap linii = (Bitmap) pictureBox1.Image;
            //draw img1 to upper left corner
            q.DrawImage(result, 0, 0);

            //draw img2 under img1, left side
            q.DrawImage(linii, 0, 0);

            pictureBox1.Image = display; //Setam un bitmap ca sursa la pictureBox1
